...
<div class="content-header row">
    <div class="content-header-left col-md-9 col-12 mb-2">
        <div class="row breadcrumbs-top">
            <div class="col-12">
                <h2 class="content-header-title float-left mb-0" style="color: #B79C6D; font-weight: bold;">تعديل التخصص</h2>
        ...
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">
                    ...
                    تعديل بيانات التخصص: {{ $specialization->name }}
                </h4>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.specializations.update', $specialization->id) }}" method="POST" id="specializationForm">
                    @csrf
                    @method('PATCH')
                                ...
                                <div class="card-body">
                                    <div class="nav-tabs-custom">
                                        ...
                                        <div class="tab-content" id="translationTabsContent">
                                            @foreach(config('app.available_locales') as $locale => $name)
                                                @php
                                                    $translation = $specialization->translations()->where('locale', $locale)->first();
                                                @endphp
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <div class="info-item">
                                        ...
                                        <span class="text-muted">{{ $specialization->created_at->format('Y-m-d H:i') }}</span>
                                    </div>
                                    <div class="info-item mt-2">
                                        <strong>آخر تحديث:</strong>
                                        <span class="text-muted">{{ $specialization->updated_at->format('Y-m-d H:i') }}</span>
                                    </div>
                                    @if($specialization->categories->count() > 0)
                                        <div class="info-item mt-2">
                                            <strong>عدد الفئات:</strong>
                                            <span class="badge badge-info">{{ $specialization->categories->count() }}</span>
                                        </div>
                                    @endif
                                </div>
                            </div>
